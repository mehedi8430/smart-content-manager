...
export function KanbanBoardTest({
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    setPosts((posts) => {
      ...
      const reorderedPosts = arrayMove(posts, itemOriginalPos, itemNewPos);

      // Update the order on the server
      const movedPost = reorderedPosts[itemNewPos];
      if (movedPost) {
        updatePostAction(campaignId, movedPost.id, { order: itemNewPos })
          .then((result) => {
            if (result.success) {
              router.refresh();
            }
